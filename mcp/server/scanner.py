...
warnings.filterwarnings(
    "ignore", category=DeprecationWarning, message=r".*ast\.Bytes.*"
)
warnings.filterwarnings(
...
async def check_security(code: str) -> List[SecurityIssue]:
    ...
    try:
        ...
        mgr = manager.BanditManager(
            b_conf, "file", debug=True, verbose=True, quiet=False
        )
...
async def count_code_metrics(code: str) -> CodeMetrics:
    ...
    return CodeMetrics(
        ...
        comment_ratio=round(
            comment_lines / total_lines * 100 if total_lines > 0 else 0, 2
        ),
    )
