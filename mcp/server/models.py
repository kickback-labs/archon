...
class DiagramGenerateRequest(BaseModel):
    ...
    code: str = Field(
        ..., description="Python code string using the diagrams package DSL"
    )
    ...
    timeout: int = Field(
        90, description="Timeout in seconds for diagram generation", ge=1, le=300
    )
...
class DiagramGenerateResponse(BaseModel):
    ...
    image_data: Optional[str] = Field(
        default=None, description="Base64-encoded PNG image data"
    )
    mime_type: Optional[str] = Field(default=None, description="MIME type of the image")
